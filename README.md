...
How It Works
data_generator.py simulates a live tick feed, producing a price update for a random ticker on each call
pipeline.py consumes the feed, computes a rolling z-score for each incoming tick against that ticker's recent history, flags anomalies, and writes everything to market_data.db
dashboard.py reads the database and generates dashboard.html, an interactive chart per ticker with anomalies marked in red
Setup & Usage
bash
# Clone the repo
